...
void CCharacterPhysicsSupport::in_NetSpawn(CSE_Abstract* e)
{
    ...
    IKinematicsAnimated* ka = pVisual ? pVisual->dcast_PKinematicsAnimated() : nullptr;
    if (!ka)
        ka = smart_cast<IKinematicsAnimated*>(pVisual);
    IKinematics* pK = pVisual ? pVisual->dcast_PKinematics() : nullptr;
    if (!pK)
        pK = smart_cast<IKinematics*>(pVisual);
    VERIFY(&e->spawn_ini());
...
IC void CCharacterPhysicsSupport::UpdateDeathAnims()
{
    ...
    {
        DestroyIKController();
        IKinematicsAnimated* ka_death = m_EntityAlife.Visual() ? m_EntityAlife.Visual()->dcast_PKinematicsAnimated() : nullptr;
        if (!ka_death)
            ka_death = smart_cast<IKinematicsAnimated*>(m_EntityAlife.Visual());
        if (ka_death)
            ka_death->PlayCycle("death_init");
...
void CCharacterPhysicsSupport::CreateSkeleton(CPhysicsShell*& pShell)
{
    ...
    IKinematics* k = m_EntityAlife.Visual() ? m_EntityAlife.Visual()->dcast_PKinematics() : nullptr;
    if (!k)
        k = smart_cast<IKinematics*>(m_EntityAlife.Visual());
    ...
    if (k && k->LL_UserData())
        disable_params.Load(k->LL_UserData());
...
{
    IKinematicsAnimated* KA = m_EntityAlife.Visual() ? m_EntityAlife.Visual()->dcast_PKinematicsAnimated() : nullptr;
    if (!KA)
        KA = smart_cast<IKinematicsAnimated*>(m_EntityAlife.Visual());
    if (m_ik_controller)
    {
        DestroyIKController();
        if (KA)
