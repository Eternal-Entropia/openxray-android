...
{
    Msg(">>> [CLevel::remove_objects] Start, initial Objects.o_count()=%d", Objects.o_count());
    ...
    while (loop)
    {
        ...
        {
            Msg(">>> [CLevel::remove_objects] All objects removed after loop=%d", 5 - loop + 1);
            break;
        }
        else
        {
            --loop;
            Msg(">>> [CLevel::remove_objects] Objects remaining: %d, loops left: %d", Objects.o_count(), loop);
        }
    }
    ...
    Msg(">>> [CLevel::remove_objects] End, final Objects.o_count()=%d", Objects.o_count());
}
