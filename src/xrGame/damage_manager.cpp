...
void CDamageManager::init_bones(LPCSTR section, CInifile const* ini)
{
    ...
    if (!kinematics)
    {
        Msg("! CDamageManager::init_bones: Visual() is not IKinematics for [%s]", m_object ? m_object->cNameSect().c_str() : "unknown");
        return;
    }
...
void CDamageManager::load_section(LPCSTR section, CInifile const* ini)
{
    ...
    if (!kinematics)
    {
        Msg("! CDamageManager::load_section: Visual() is not IKinematics for [%s]", m_object ? m_object->cNameSect().c_str() : "unknown");
        return;
    }
...
void CDamageManager::HitScale(const int element, float& hit_scale, float& wound_scale, bool aim_bullet)
{
    ...
    IKinematics* V = m_object && m_object->Visual() ? m_object->Visual()->dcast_PKinematics() : nullptr;
    if (!V && m_object && m_object->Visual())
        V = smart_cast<IKinematics*>(m_object->Visual());
    if (!V)
    {
        hit_scale = 1.f * m_default_hit_factor;
        wound_scale = 1.f * m_default_wound_factor;
