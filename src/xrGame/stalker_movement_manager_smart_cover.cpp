...
void stalker_movement_manager_smart_cover::modify_animation(CBlend* blend)
{
    ...
    IKinematicsAnimated* ka = object().Visual() ? object().Visual()->dcast_PKinematicsAnimated() : nullptr;
    if (!ka)
        return;
    CMotionDef* motion_def = ka->LL_GetMotionDef(blend->motionID);
    VERIFY(motion_def);
...
void stalker_movement_manager_smart_cover::reach_enter_location(u32 const& time_delta)
{
    ...
    IKinematicsAnimated* ka = object().Visual() ? object().Visual()->dcast_PKinematicsAnimated() : nullptr;
    m_enter_animation =
        ka ? ka->ID_Cycle(current_transition().animation().animation_id()) : MotionID();
