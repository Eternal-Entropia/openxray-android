...
void CControlAnimationBase::SelectVelocities()
{
    ...
    EMotionAnim cur_motion = cur_anim_info().get_motion();
    SAnimItem* item_it = (cur_motion < m_anim_storage.size()) ? m_anim_storage[cur_motion] : nullptr;
    if (!item_it)
    {
        stop_now();
        return;
    ...
    {
        cur_motion = cur_anim_info().get_motion();
        item_it = (cur_motion < m_anim_storage.size()) ? m_anim_storage[cur_motion] : nullptr;
        if (!item_it)
            return;
