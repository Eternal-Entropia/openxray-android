    ...
    float cur_speed, EMotionAnim target_anim, EMotionAnim& new_anim, float& a_speed)
{
    ...
    for (auto I = B; I != E; ++I)
    {
        ...
        if (_abs(tmp) >= 1000.f)
            tmp = 1.0f;
        a_speed = (best_param->velocity.linear > EPS) ? (tmp * cur_speed / best_param->velocity.linear) : 1.0f;
        if (_abs(a_speed) >= 1000.f)
            a_speed = 1.0f;
        return true;
    }
    return false;
...
bool CControlAnimationBase::accel_chain_test()
{
    ...
    for (auto I = m_accel.chain.begin(); I != m_accel.chain.end(); ++I)
    {
        if (I->size() < 2)
            continue;
        if (*(I->begin()) >= m_anim_storage.size())
            continue;
        ...
        for (auto IT = I->begin() + 1; IT != I->end(); ++IT)
        {
            ...
            if (to >= from)
            {
                Msg("! Incompatible speed ranges. Monster[%s] From animation [%s] To animation [%s]",
