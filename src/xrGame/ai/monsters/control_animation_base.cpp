...
void CControlAnimationBase::select_animation(bool anim_end)
{
    ...
    if (cur_anim_info().get_motion() >= m_anim_storage.size())
        return;
    ...
    if (m_override_animation == cur_anim_info().get_motion() && m_override_animation_index != (u32)-1)
    {
        if (anim_it->count == 0)
            return;
        index = (m_override_animation_index < anim_it->count) ? m_override_animation_index : 0;
    }
    ...
    if (!cur_anim.valid())
    {
        Msg("! Monster [%s] missing animation [%s]", m_object->cName().c_str(), s2);
        SAnimItem* idle_it = m_anim_storage.empty() ? nullptr : m_anim_storage[0];
        if (idle_it && idle_it->count > 0)
        {
            cur_anim = smart_cast<IKinematicsAnimated*>(m_object->Visual())
                ->ID_Cycle_Safe(strconcat(sizeof(s2), s2, idle_it->target_name.c_str(), "0"));
        }
        if (!cur_anim.valid())
            return;
    }
...
void CControlAnimationBase::UpdateAnimCount()
{
    ...
    for (auto it = m_anim_storage.begin(); it != m_anim_storage.end(); ++it)
    {
        ...
        {
            xr_sprintf(s, "Notice: No animation: %s for monster %s", (*it)->target_name.c_str(), m_object->cName().c_str());
            Msg("! %s", s);
