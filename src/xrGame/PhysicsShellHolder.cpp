...
void CPhysicsShellHolder::activate_physic_shell()
{
    ...
    if (H_Parent() && H_Parent()->Visual() && H_Parent()->Visual()->dcast_PKinematics())
    {
        H_Parent()->Visual()->dcast_PKinematics()->CalculateBones_Invalidate();
        H_Parent()->Visual()->dcast_PKinematics()->CalculateBones(TRUE);
    }
