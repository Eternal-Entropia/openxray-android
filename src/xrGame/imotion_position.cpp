...
static void interactive_motion_diag(LPCSTR message, const CBlend& b, CPhysicsShell* s, float time_left)
{
    ...
    VERIFY(m.valid());
    VERIFY(s);
    IKinematicsAnimated* KA = s->PKinematics() ? s->PKinematics()->dcast_PKinematicsAnimated() : nullptr;
    VERIFY(KA);
    CPhysicsShellHolder* O = s->get_ElementByStoreOrder(0)->PhysicsRefObject() ? s->get_ElementByStoreOrder(0)->PhysicsRefObject()->IObject()->cast_physics_shell_holder() : nullptr;
    VERIFY(O);
...
void imotion_position::state_start()
{
    ...
    IKinematicsAnimated* KA = shell->PKinematics() ? shell->PKinematics()->dcast_PKinematicsAnimated() : nullptr;
    VERIFY(KA);
...
void imotion_position::rootbone_callback(CBoneInstance* BI)
{
    ...
    VERIFY(K);
    IKinematicsAnimated* KA = K ? K->dcast_PKinematicsAnimated() : nullptr;
    VERIFY(KA);
    SKeyTable keys;
