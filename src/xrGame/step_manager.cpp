...
void CStepManager::reload(LPCSTR section)
{
    ...
    IKinematicsAnimated* skeleton_animated = m_object && m_object->Visual() ? m_object->Visual()->dcast_PKinematicsAnimated() : nullptr;
    if (!skeleton_animated && m_object && m_object->Visual())
        skeleton_animated = smart_cast<IKinematicsAnimated*>(m_object->Visual());

    if (!skeleton_animated)
    {
        Msg("! (CStepManager::reload) object is not animated: %s", m_object ? m_object->cNameVisual().c_str() : "unknown");
        return;
    }
#ifdef DEBUG
    ...
    for (u32 i = 0; pSettings->r_line(anim_section, i, &anim_name, &val); ++i)
    {
        ...
        if (!motion_id)
        {
            ...
            IKinematicsAnimated* KA = skeleton_animated;
...
void CStepManager::on_animation_start(MotionID motion_id, CBlend* blend)
{
    ...
    if (it == m_steps_map.end())
    {
        ...
        if (debug_step_info)
        {
            IKinematicsAnimated* KA = m_object && m_object->Visual() ? m_object->Visual()->dcast_PKinematicsAnimated() : nullptr;
            if (KA)
            {
                std::pair<LPCSTR, LPCSTR> anim_name = KA->LL_MotionDefName_dbg(motion_id);
...
Fvector CStepManager::get_foot_position(ELegType leg_type)
{
    ...
    IKinematics* pK = m_object && m_object->Visual() ? m_object->Visual()->dcast_PKinematics() : nullptr;
    if (!pK && m_object && m_object->Visual())
        pK = smart_cast<IKinematics*>(m_object->Visual());
    if (!pK)
        return m_object ? m_object->Position() : Fvector();
...
{
    IKinematics* kinematics = m_object && m_object->Visual() ? m_object->Visual()->dcast_PKinematics() : nullptr;
    if (!kinematics && m_object && m_object->Visual())
        kinematics = smart_cast<IKinematics*>(m_object->Visual());
    if (!kinematics)
        return;
    ...
    for (auto I = data.Data.cbegin(); I != data.Data.cend(); ++I)
    {
        ...
        u16 index = kinematics->LL_BoneID(item.second.c_str());
...
void CStepManager::reload_foot_bones()
{
    ...
    CInifile* ini = kinematics ? kinematics->LL_UserData() : nullptr;
    ...
    else
    {
        if (pSettings->line_exist(m_object->cNameSect().c_str(), "foot_bones"))
            ...
            Msg("! (CStepManager::reload_foot_bones) section [foot_bones] not found in user_data or system.ltx for [%s]", m_object->cNameSect().c_str());
    }
