...
void CUIActorMenu::InitInventoryContents(CUIDragDropListEx* pBagList, bool onlyBagList /*= false*/)
{
    ...
    for (size_t i = 0; i < inv.m_ruck.size();)
    {
        PIItem item = inv.m_ruck[i];
        u16 slot = item->BaseSlot();
        if (slot != NO_ACTIVE_SLOT && inv.SlotIsPersistent(slot) && inv.CanPutInSlot(item, slot))
        {
            inv.Slot(slot, item, true, false);
        }
        else
        {
            ++i;
    ...
    if (onlyBagList)
        inv.AddAvailableItems(ruck_list, true);
    else
        ruck_list = inv.m_ruck;
...
bool CUIActorMenu::ToSlot(CUICellItem* itm, bool force_place, u16 slot_id)
{
    ...
    if (m_pActorInvOwner->inventory().CanPutInSlot(iitem, slot_id))
    {
        ...
        CUICellItem* i = old_owner ? old_owner->RemoveItem(itm, (old_owner == new_owner)) : itm;
