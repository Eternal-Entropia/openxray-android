...
CGamePersistent::CGamePersistent()
{
    ...
    if (DofValue)
        ...
        SetBaseDof(Fvector3().set(0.0f, 0.0f, 0.0f));
}
...
void CGamePersistent::update_logo_intro()
{
    ...
    if (notLoadingLevel)
    {
        Msg("* CGamePersistent::update_logo_intro(): intro finished, activating MainMenu");
        m_pMainMenu->Activate(true);
    }
    else
    {
        Msg("* CGamePersistent::update_logo_intro(): intro finished, level loading/running, skipping MainMenu");
    }
}
...
void CGamePersistent::OnEvent(EVENT E, u64 P1, u64 P2)
{
    ...
    if (E == eQuickLoad)
    {
        ...
        Msg(">>> [GamePersistent] OnEvent eQuickLoad fired for saved_name: '%s'", saved_name ? saved_name : "<null>");

        Msg(">>> [GamePersistent] Calling Level().remove_objects()...");
        Level().remove_objects();
        game_sv_Single* game = smart_cast<game_sv_Single*>(Level().Server->GetGameState());
        R_ASSERT(game);
        Msg(">>> [GamePersistent] Calling game->restart_simulator('%s')...", saved_name);
        game->restart_simulator(saved_name);
        Msg(">>> [GamePersistent] game->restart_simulator completed successfully!");
        xr_free(saved_name);
        return;
    }
    if (E == eDemoStart)
