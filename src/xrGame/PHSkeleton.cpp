...
{
    IKinematics* K = PPhysicsShellHolder()->Visual() ? PPhysicsShellHolder()->Visual()->dcast_PKinematics() : nullptr;
    if (K)
    {
        K->LL_SetBoneRoot(0);
...
void CPHSkeleton::UnsplitSingle(CPHSkeleton* SO)
{
    ...
    IKinematics* newKinematics = O->Visual() ? O->Visual()->dcast_PKinematics() : nullptr;
    IKinematics* pKinematics = obj->Visual() ? obj->Visual()->dcast_PKinematics() : nullptr;
...
bool CPHSkeleton::ReadyForRemove()
{
    ...
    IKinematics* K = obj->Visual() ? obj->Visual()->dcast_PKinematics() : nullptr;
    if (K)
        RecursiveBonesCheck(K->LL_GetBoneRoot());
    return removable;
}
