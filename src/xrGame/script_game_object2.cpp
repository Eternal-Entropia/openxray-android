...
{
    IKinematicsAnimated* sa = object().Visual() ? object().Visual()->dcast_PKinematicsAnimated() : nullptr;
    if (sa)
    {
...
void CScriptGameObject::Hit(CScriptHit* tpLuaHit)
{
    ...
    IKinematics* V = object().Visual() ? object().Visual()->dcast_PKinematics() : nullptr;
    VERIFY(V);
