...
CMainMenu::CMainMenu()
{
    ...
    class CStartedEventCb : public CEventNotifierCallbackWithCid
    {
        CMainMenu* m_mainmenu;

    public:
        CStartedEventCb(CID cid, CMainMenu* mm) : CEventNotifierCallbackWithCid(cid), m_mainmenu(mm) {}
        void ProcessEvent() override
        {
    ...
    m_script_started_event_cid = ai().template Subscribe<CStartedEventCb>(CAI_Space::EVENT_SCRIPT_ENGINE_STARTED, this);
...
CMainMenu::~CMainMenu()
{
    ...
    ai().Unsubscribe(m_script_started_event_cid, CAI_Space::EVENT_SCRIPT_ENGINE_STARTED);
}

void CMainMenu::Activate(bool bActivate)
{
    Msg("* CMainMenu::Activate(%s)", bActivate ? "true" : "false");
    if (m_Flags.test(flActive) == bActivate)
    {
        Msg("* CMainMenu::Activate: already in state %s, ignoring", bActivate ? "true" : "false");
        return;
    }
    ...
    if (bActivate)
    {
        ...
        if (!ReloadUI())
        {
            Msg("! CMainMenu::Activate: ReloadUI() failed!");
            return;
        }
...
bool CMainMenu::ReloadUI()
{
    ...
    Msg("* CMainMenu::ReloadUI(): instantiating MAIN_MNU...");
    IFactoryObject* dlg = NEW_INSTANCE(xray::make_clsid("MAIN_MNU"));
    if (!dlg)
    {
        Msg("! CMainMenu::ReloadUI(): NEW_INSTANCE(MAIN_MNU) returned nullptr, attempting emergency registration...");
        const_cast<CObjectFactory*>(&object_factory())->register_script_class("ui_main_menu.main_menu", "MAIN_MNU", "main_menu");
        dlg = NEW_INSTANCE(xray::make_clsid("MAIN_MNU"));
    }
    if (!dlg)
    {
        Msg("! CMainMenu::ReloadUI(): emergency registration also failed!");
    ...
    if (!m_startDialog)
    {
        Msg("! CMainMenu::ReloadUI(): smart_cast to CUIDialogWnd failed!");
        xr_delete(dlg);
        m_Flags.set(flActive | flNeedChangeCapture, FALSE);
        return false;
    }
    ...
    Msg("* CMainMenu::ReloadUI(): MAIN_MNU created and ShowDialog(true) called successfully!");
    return true;
}
...
void CMainMenu::OnRender()
{
    ...
    static bool s_first_menu_render = true;
    if (s_first_menu_render)
    {
        s_first_menu_render = false;
        Msg("* CMainMenu::OnRender(): first menu render frame (OnRenderPPUI_query=%d)", (int)OnRenderPPUI_query());
    }
...
void CMainMenu::OnRenderPPUI_main()
{
    ...
    static bool s_first_ppui_render = true;
    if (s_first_ppui_render)
    {
        s_first_ppui_render = false;
        Msg("* CMainMenu::OnRenderPPUI_main(): first PPUI render frame (has dialogs=%d)", (int)(m_startDialog != nullptr));
    }
