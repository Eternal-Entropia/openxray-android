...
{
    Msg(">>> [CALifeSimulator] Constructor entered: command_line='%s'", command_line ? command_line->c_str() : "<null>");
    // XXX: why do we need to reinitialize script engine?
    if (!strstr(Core.Params, "-keep_lua"))
    {
        Msg(">>> [CALifeSimulator] Restarting script engine...");
        ai().RestartScriptEngine();
        Msg(">>> [CALifeSimulator] Script engine restarted!");
    }
    ...
    Msg(">>> [CALifeSimulator] params: game_or_spawn='%s', game_type='%s', alife='%s', new_or_load='%s'",
        p.m_game_or_spawn, p.m_game_type, p.m_alife, p.m_new_or_load);
    ...
    *command_line = temp;
    ...
    const bool isNewGame = !xr_strcmp(p.m_new_or_load, "new");

    LPCSTR start_game_callback = pSettings->r_string(alife_section, "start_game_callback");
    Msg(">>> [CALifeSimulator] Calling start_game_callback '%s' (isNewGame=%d)...", start_game_callback, (int)isNewGame);
    luabind::functor<void> functor;
    R_ASSERT2(GEnv.ScriptEngine->functor(start_game_callback, functor), "failed to get start game callback");
    functor(isNewGame);
    Msg(">>> [CALifeSimulator] start_game_callback finished! Now calling load('%s')...", p.m_game_or_spawn);

    load(p.m_game_or_spawn, !xr_strcmp(p.m_new_or_load, "load") ? false : true, !xr_strcmp(p.m_new_or_load, "new"));
    Msg(">>> [CALifeSimulator] load() completed successfully!");
}
