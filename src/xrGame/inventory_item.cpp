...
{
    const u16 config_base_slot = m_ItemCurrPlace.base_slot_id;
    m_ItemCurrPlace.value = packet.r_u16();

    // Preserve valid base_slot_id from section config
    if (m_ItemCurrPlace.base_slot_id == 0 || config_base_slot != 0)
        m_ItemCurrPlace.base_slot_id = config_base_slot;

    if (ShadowOfChernobylMode && m_ItemCurrPlace.type == eItemPlaceSlot)
    {
        if (m_ItemCurrPlace.slot_id == 0 && m_ItemCurrPlace.base_slot_id != 0)
        {
            m_ItemCurrPlace.slot_id = m_ItemCurrPlace.base_slot_id;
        }
        else if (m_ItemCurrPlace.slot_id < m_ItemCurrPlace.base_slot_id &&
                 (m_ItemCurrPlace.slot_id + 1) == m_ItemCurrPlace.base_slot_id)
        {
            m_ItemCurrPlace.slot_id = m_ItemCurrPlace.base_slot_id;
        }
    }
...
void CInventoryItem::UpdateXForm()
{
    ...
    R_ASSERT(E);
    IKinematics* V = E->Visual() ? E->Visual()->dcast_PKinematics() : nullptr;
    if (!V)
        return;
