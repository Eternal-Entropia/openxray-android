...
bool CScriptEntity::bfAssignAnimation(CScriptEntityAction* tpEntityAction)
{
    ...
    IKinematicsAnimated* pVisualObject = object().Visual() ? object().Visual()->dcast_PKinematicsAnimated() : nullptr;
    if (!pVisualObject)
        return false;
    m_tpNextAnimation = pVisualObject->ID_Cycle_Safe(GetCurrentAction()->m_tAnimationAction.m_caAnimationToPlay.c_str());
...
//%s",Device.dwTimeGlobal,*GetCurrentAction()->m_tAnimationAction.m_caAnimationToPlay, *object().cName());
        ...
        IKinematicsAnimated* skeleton_animated = object().Visual() ? object().Visual()->dcast_PKinematicsAnimated() : nullptr;
