...
bool CProjector::net_Spawn(CSE_Abstract* DC)
{
    ...
    //////////////////////////////////////////////////////////////////////////
    CBoneInstance& b_x = K->LL_GetBoneInstance(bone_x.id);
    b_x.set_callback(bctCustom, BoneCallbackX, this);

    CBoneInstance& b_y = K->LL_GetBoneInstance(bone_y.id);
...
void CProjector::TurnOn()
{
    ...
    IKinematics* visual = Visual() ? Visual()->dcast_PKinematics() : nullptr;
    if (!visual)
        return;
...
void CProjector::TurnOff()
{
    ...
    IKinematics* visual = Visual() ? Visual()->dcast_PKinematics() : nullptr;
    if (visual)
        visual->LL_SetBoneVisible(guid_bone, FALSE, TRUE);
}
...
void CProjector::UpdateCL()
{
    ...
    if (light_render->get_active())
    {
        ...
        if (!visual)
            return;
        CBoneInstance& BI = visual->LL_GetBoneInstance(guid_bone);
        Fmatrix M;
