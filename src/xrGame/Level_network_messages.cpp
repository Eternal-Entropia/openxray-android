...
//					Msg("! TimeServer[%d] < TimeReceive[%d]", Level().timeServer(), P->timeReceive);
        ...
        case M_CHANGE_LEVEL: {
            ...
            if (m_type == M_LOAD_GAME)
            {
                ...
                Msg(">>> [Level_network_messages] M_LOAD_GAME received for: '%s' (alife=%p)", saved_name, ai().get_alife());
                if (xr_strlen(saved_name) && ai().get_alife())
                {
                    CSavedGameWrapper wrapper(saved_name);
                    const auto save_lvl_id = wrapper.level_id();
                    const auto cur_lvl_id = ai().level_graph().level_id();
                    Msg(">>> [Level_network_messages] save level_id=%u, current level_id=%u", (u32)save_lvl_id, (u32)cur_lvl_id);
                    if (save_lvl_id == cur_lvl_id)
                    {
                        Msg(">>> [Level_network_messages] Same level! Deferring Game:QuickLoad for '%s'...", saved_name);
                        Engine.Event.Defer("Game:QuickLoad", size_t(xr_strdup(saved_name)), 0);
                        break;
                    }
                    else
                    {
                        Msg(">>> [Level_network_messages] Different level (%u != %u)! Calling MakeReconnect...", (u32)save_lvl_id, (u32)cur_lvl_id);
                    }
                }
            }
            Msg(">>> [Level_network_messages] Calling MakeReconnect()...");
            MakeReconnect();
        }
        break;
