...
#endif
                        const bool realistic_hit = pSettingsOpenXRay && pSettingsOpenXRay->read_if_exists<bool>("gameplay", "realistic_hit_probability", true);
                        if (!realistic_hit && Random.randF(0.f, 1.f) > (ahp * hpf))
                        {
...
void CBulletManager::DynamicObjectHit(CBulletManager::_event& E)
{
    ...
    // bone-space
    IKinematics* V = E.R.O->Visual() ? E.R.O->Visual()->dcast_PKinematics() : nullptr;
