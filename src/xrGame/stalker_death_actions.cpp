...
void CStalkerActionDead::initialize()
{
    ...
    u16 active_weapon_slot = object().inventory().GetActiveSlot();
    if (active_weapon_slot == NO_ACTIVE_SLOT)
    {
        if (object().inventory().ItemFromSlot(INV_SLOT_3))
            active_weapon_slot = INV_SLOT_3;
        else if (object().inventory().ItemFromSlot(INV_SLOT_2))
            active_weapon_slot = INV_SLOT_2;
    }
...
void CStalkerActionDead::execute()
{
    ...
    {
        if (object().inventory().ItemFromSlot(INV_SLOT_3))
            active_slot = INV_SLOT_3;
        else if (object().inventory().ItemFromSlot(INV_SLOT_2))
            active_slot = INV_SLOT_2;
    }
