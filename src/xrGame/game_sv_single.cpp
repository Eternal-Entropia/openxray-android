...
{
    Msg(">>> [game_sv_Single] restart_simulator entered for: '%s'", saved_game_name ? saved_game_name : "<null>");
    shared_str& options = *alife().server_command_line();
    Msg(">>> [game_sv_Single] options='%s'", options.c_str());

    Msg(">>> [game_sv_Single] Deleting m_alife_simulator...");
    delete_data(m_alife_simulator);
    Msg(">>> [game_sv_Single] server().clear_ids()...");
    server().clear_ids();
    ...
    Msg(">>> [game_sv_Single] Calling xr_new<CALifeSimulator>...");
    m_alife_simulator = xr_new<CALifeSimulator>(&server(), &options);
    Msg(">>> [game_sv_Single] CALifeSimulator created! PreCache(60, true)...");
    g_pGamePersistent->LoadTitle("st_client_synchronising");
#ifdef __ANDROID__
    Device.PreCache(60, false);
#else
    Device.PreCache(60, true);
#endif
    Msg(">>> [game_sv_Single] PreCache done, calling LoadEnd...");
    g_pGamePersistent->LoadEnd();
    Msg(">>> [game_sv_Single] restart_simulator fully finished!");
}
