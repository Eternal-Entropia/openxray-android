...
void CPHDestroyable::NotificatePart(CPHDestroyableNotificate* dn)
{
    ...
    IKinematics* own_K = PPhysicsShellHolder()->Visual() ? PPhysicsShellHolder()->Visual()->dcast_PKinematics() : nullptr;
    IKinematics* new_K = dn->PPhysicsShellHolder()->Visual() ? dn->PPhysicsShellHolder()->Visual()->dcast_PKinematics() : nullptr;
