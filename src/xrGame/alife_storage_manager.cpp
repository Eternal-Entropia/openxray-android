...
{
    Msg(">>> [CALifeStorageManager::load_buffer] entered, buffer_size=%u, file_name='%s'", buffer_size, file_name ? file_name : "<null>");
    ...
    Msg(">>> [CALifeStorageManager::load_buffer] loading header...");
    header().load(source);
    Msg(">>> [CALifeStorageManager::load_buffer] loading time_manager...");
    time_manager().load(source);
    Msg(">>> [CALifeStorageManager::load_buffer] loading spawns...");
    spawns().load(source, file_name);
    Msg(">>> [CALifeStorageManager::load_buffer] calling graph().on_load()...");
    graph().on_load();
    Msg(">>> [CALifeStorageManager::load_buffer] loading objects...");
    objects().load(source);

    VERIFY(can_register_objects());
    can_register_objects(false);

    Msg(">>> [CALifeStorageManager::load_buffer] performing IDgen and register for %u objects...", (u32)objects().objects().size());
    ...
    Msg(">>> [CALifeStorageManager::load_buffer] loading registry...");
    registry().load(source);

    can_register_objects(true);

    Msg(">>> [CALifeStorageManager::load_buffer] calling on_register for objects...");
    for (auto& object : objects().objects())
        object.second->on_register();

    if (!g_pGameLevel)
    {
        Msg(">>> [CALifeStorageManager::load_buffer] g_pGameLevel is null, returning early");
        return;
    }

    Msg(">>> [CALifeStorageManager::load_buffer] calling autosave_manager().on_game_loaded()...");
    ...
    if (GEnv.ScriptEngine->functor("alife_storage_manager.CALifeStorageManager_after_load", funct2))
        funct2(file_name);
	//-Neloreck
    Msg(">>> [CALifeStorageManager::load_buffer] completed successfully!");
}

bool CALifeStorageManager::load(LPCSTR save_name_no_check)
{
    Msg(">>> [CALifeStorageManager::load_file] entered with save_name_no_check='%s'", save_name_no_check ? save_name_no_check : "<null>");
    pcstr gameSaveExtension = SAVE_EXTENSION;
    if (ShadowOfChernobylMode || ClearSkyMode)
        gameSaveExtension = SAVE_EXTENSION_LEGACY;
    ...
    if (!save_name[0])
    {
        ...
        {
            Log("There is no file name specified!");
            Msg(">>> [CALifeStorageManager::load_file] ERROR: No file name specified!");
            return false;
        }
    }
    else
    ...
    Msg(">>> [CALifeStorageManager::load_file] resolved path: '%s'", file_name);
    ...
    Msg(">>> [CALifeStorageManager::load_file] save file opened, length=%u bytes", (u32)stream->length());
    ...
    Msg(">>> [CALifeStorageManager::load_file] gameSaveIsValid=%d", (int)gameSaveIsValid);
    ...
    Msg(">>> [CALifeStorageManager::load_file] Calling unload() and reload('%s')...", m_section ? m_section : "<null>");
    unload();
    reload(m_section);
    ...
    Msg(">>> [CALifeStorageManager::load_file] Calling rtc_decompress: source_count=%u, compressed_size=%u...", source_count, compressed_size);
    rtc_decompress(source_data, source_count, stream->pointer(), compressed_size);
    Msg(">>> [CALifeStorageManager::load_file] rtc_decompress done! Closing stream and calling load(source_data)...");
    FS.r_close(stream);
    load(source_data, source_count, file_name);
    xr_free(source_data);

    Msg(">>> [CALifeStorageManager::load_file] groups().on_after_game_load()...");
    groups().on_after_game_load();

    VERIFY(graph().actor());

    Msg("* Game %s is successfully loaded from file '%s' (%.3fs)", save_name, file_name, timer.GetElapsed_sec());
    Msg(">>> [CALifeStorageManager::load_file] finished successfully!");
