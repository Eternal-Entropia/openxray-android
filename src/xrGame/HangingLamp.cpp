...
bool CHangingLamp::net_Spawn(CSE_Abstract* DC)
{
    ...
    CPHSkeleton::Spawn(e);
    IKinematicsAnimated* K_anim = Visual() ? Visual()->dcast_PKinematicsAnimated() : nullptr;
    if (K_anim)
        K_anim->PlayCycle("idle");
    IKinematics* K_kin = Visual() ? Visual()->dcast_PKinematics() : nullptr;
    if (K_kin)
    {
        K_kin->CalculateBones_Invalidate();
        K_kin->CalculateBones(TRUE);
        //.intepolate_pos
    }
