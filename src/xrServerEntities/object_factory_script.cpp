...
void CObjectFactory::register_script_class(LPCSTR unknown_class, LPCSTR clsid, LPCSTR script_clsid)
{
    ...
    {
        Msg("! CObjectFactory: FAILED to register script class '%s' for clsid '%s'!", unknown_class, clsid);
        GEnv.ScriptEngine->script_log(LuaMessageType::Error, "Cannot register class %s", unknown_class);
        return;
    }
    Msg("* CObjectFactory: registered script class '%s' -> clsid '%s'", unknown_class, clsid);
...
void CObjectFactory::register_script_classes()
{
    ...
    if (strstr(Core.Params, "-soc") || strstr(Core.Params, "-shoc"))
    {
        if (!item(TEXT2CLSID("MAIN_MNU"), true))
        {
            Msg("* CObjectFactory: auto-registering SoC MAIN_MNU...");
            register_script_class("ui_main_menu.main_menu", "MAIN_MNU", "main_menu");
        }
    }
}
