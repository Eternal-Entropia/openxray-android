...
#ifndef SMART_CAST_FUNCTIONS_DEFINED
#define SMART_CAST_FUNCTIONS_DEFINED
...
namespace detail
{
    ...
    inline Target smart_cast_impl(Source* p)
    {
        if (!p)
            return nullptr;

        using RawTarget = std::remove_cv_t<std::remove_pointer_t<Target>>;
        using RawSource = std::remove_cv_t<Source>;

        if constexpr (std::is_same_v<RawTarget, RawSource>)
        {
            return const_cast<Target>(p);
        }
        else if constexpr (std::is_same_v<RawTarget, IKinematics> && std::is_same_v<RawSource, IRenderVisual>)
        {
            return const_cast<RawSource*>(p)->dcast_PKinematics();
        }
        else if constexpr (std::is_same_v<RawTarget, IKinematicsAnimated> && std::is_same_v<RawSource, IRenderVisual>)
        {
            return const_cast<RawSource*>(p)->dcast_PKinematicsAnimated();
        }
        else if constexpr (std::is_same_v<RawTarget, IParticleCustom> && std::is_same_v<RawSource, IRenderVisual>)
        {
            return const_cast<RawSource*>(p)->dcast_ParticleCustom();
        }
        else if constexpr (std::is_same_v<RawTarget, IKinematics> && std::is_same_v<RawSource, IKinematicsAnimated>)
        {
            return const_cast<RawSource*>(p)->dcast_PKinematics();
        }
        else if constexpr (std::is_same_v<RawTarget, IKinematicsAnimated> && std::is_same_v<RawSource, IKinematics>)
        {
            return const_cast<RawSource*>(p)->dcast_PKinematicsAnimated();
        }
        else if constexpr (std::is_same_v<RawTarget, IRenderVisual> && std::is_same_v<RawSource, IKinematics>)
        {
            return const_cast<RawSource*>(p)->dcast_RenderVisual();
        }
        else if constexpr (std::is_same_v<RawTarget, IRenderVisual> && std::is_same_v<RawSource, IKinematicsAnimated>)
        {
            return const_cast<RawSource*>(p)->dcast_RenderVisual();
        }
        else
...
template <typename Target, typename Source>
inline Target smart_cast(Source* p)
{
    return detail::smart_cast_impl<Target>(p);
}

template <typename Target, typename Source>
inline Target smart_cast(Source& p)
{
    using PtrTarget = std::add_pointer_t<std::remove_reference_t<Target>>;
    auto res = detail::smart_cast_impl<PtrTarget>(&p);
    if (!res)
    {
        throw std::bad_cast();
    }
...
#endif // SMART_CAST_FUNCTIONS_DEFINED
