...
inline CObjectFactory::ClientObjectBaseClass* CObjectFactory::client_object(const CLASS_ID& clsid) const
{
    ...
    {
        string16 clsid_str;
        CLSID2TEXT(clsid, clsid_str);
        Msg("! CObjectFactory::client_object: no object item found for clsid '%s'", clsid_str);
        return nullptr;
    }
    ClientObjectBaseClass* result = object->client_object();
    if (!result)
    {
        string16 clsid_str;
        CLSID2TEXT(clsid, clsid_str);
        Msg("! CObjectFactory::client_object: object->client_object() returned nullptr for clsid '%s'!", clsid_str);
    }
    return result;
}
