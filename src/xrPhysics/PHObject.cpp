...
static inline CPHObject* CastToCPHObject(ISpatial* s)
{
    if (!s)
        return nullptr;
    SpatialBase* sb = s->dcast_SpatialBase();
    return sb ? static_cast<CPHObject*>(sb) : nullptr;
}
...
void CPHObject::Collide()
{
    if (m_flags.test(fl_ray_motions))
    {
        ...
        for (; E != I; ++I)
        {
            ...
            for (; i != e; ++i)
            {
                CPHObject* obj2 = CastToCPHObject(*i);
                VERIFY(obj2);
...
void CPHObject::reinit_single()
{
    ...
    for (; i != e; ++i)
    {
        CPHObject* obj = CastToCPHObject(*i);
        VERIFY(obj);
        obj->IslandReinit();
    }
    result.clear();
