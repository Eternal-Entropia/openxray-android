...
#include "script_thread.hpp"
#endif
#include "ui_main_menu_embedded.hpp"
...
#if defined(XR_PLATFORM_ANDROID) || defined(XR_PLATFORM_LINUX) || defined(XR_ARCHITECTURE_ARM64)
...
bool CScriptEngine::load_buffer_into_namespace(LPCSTR caBuffer, size_t tSize, LPCSTR caScriptName, LPCSTR caNamespaceName)
{
    int start = lua_gettop(lua());
    string_path l_caLuaFileName;
    strconcat(sizeof(l_caLuaFileName), l_caLuaFileName, "@", caScriptName);
    if (!load_buffer(lua(), caBuffer, tSize, l_caLuaFileName, caNamespaceName))
    {
        lua_settop(lua(), start);
        return false;
    }
...
void CScriptEngine::init(export_func exporter, bool loadGlobalNamespace)
{
    ...
    if (!lua())
    {
        Log("! ERROR : Cannot initialize script virtual machine in CScriptEngine::init!");
        return;
    }
    luabind::open(lua());
...
bool CScriptEngine::process_file_if_exists(LPCSTR file_name, bool warn_if_not_exist)
{
    ...
    if (m_reload_modules || (*file_name && !namespace_loaded(file_name)))
    {
        ...
        const bool is_ui_main_menu = (0 == xr_strcmp(file_name, "ui_main_menu"));
        if (!FS.exist(S, FSType::Any))
        {
            if (is_ui_main_menu)
            {
                const char* embedded = get_embedded_ui_main_menu();
                Msg("* Loading embedded fallback script: %s", S1);
                m_reload_modules = false;
                return load_buffer_into_namespace(embedded, xr_strlen(embedded), S1, *file_name ? file_name : GlobalNamespace);
            }
        ...
        m_reload_modules = false;
        if (!load_file_into_namespace(S, *file_name ? file_name : GlobalNamespace))
        {
            if (is_ui_main_menu)
            {
                Msg("! Failed to load '%s' from disk, falling back to embedded script", S);
                const char* embedded = get_embedded_ui_main_menu();
                return load_buffer_into_namespace(embedded, xr_strlen(embedded), S1, *file_name ? file_name : GlobalNamespace);
            }
            return false;
