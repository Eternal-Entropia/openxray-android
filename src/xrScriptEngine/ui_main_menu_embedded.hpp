...
function main_menu:OnButton_new_novice_game()
	local console = get_console()
	if console then console:execute("g_game_difficulty gd_novice") end
	self:StartGame()
end

function main_menu:OnButton_new_stalker_game()
	local console = get_console()
	if console then console:execute("g_game_difficulty gd_stalker") end
	self:StartGame()
end

function main_menu:OnButton_new_veteran_game()
	local console = get_console()
	if console then console:execute("g_game_difficulty gd_veteran") end
	self:StartGame()
end

function main_menu:OnButton_new_master_game()
	local console = get_console()
	if console then console:execute("g_game_difficulty gd_master") end
	self:StartGame()
end
...
function main_menu:StartGame()
	...
	if (alife() ~= nil) then
		if console then console:execute("disconnect") end
	end
	if device() then
		device():pause(false)
	end
...
function main_menu:OnButton_options_clicked()
	...
	if self.opt_dlg.SetCurrentValues ~= nil then
		...
		-- Shadow of Chernobyl names this operation UpdateControls.
	...
	if keyboard_action == ui_events.WINDOW_KEY_PRESSED then
		if dik == DIK_keys.DIK_ESCAPE or bind == key_bindings.kQUIT or (key_bindings.kUI_BACK and bind == key_bindings.kUI_BACK) then
			if level.present() then
				if (db.actor == nil or db.actor:alive() or false == IsGameTypeSingle()) then
