        ...
        if ((dwType & ~CFS_CompressMark) == ID)
        {
            ...
            if (dwType & CFS_CompressMark)
            {
                ...
                if (!result && shouldDecrypt)
                {
                    ...
                    g_trivial_encryptor.encode(src_data, dwSize, src_data); // rollback WW
                ...
                if (!result && shouldDecrypt)
                {
                    // Rollback RU key and try unencrypted (plain archive)
                    g_trivial_encryptor.encode(src_data, dwSize, src_data, trivial_encryptor::key_flag::russian);
                    result = _decompressLZ(&dest, &dest_sz, src_data, dwSize, archiveSize);
                }
...
bool ignore_path(pcstr _path)
{
    ...
    convert_path_separators(conv_path);
    struct stat st;
    bool exists = (stat(conv_path, &st) == 0);
    xr_free(conv_path);
    return !exists;
#else
#   error Select or add implementation for your platform
...
bool CLocatorAPI::Recurse(pcstr path)
{
    ...
    string_path cleanPath = { 0 };
    xr_strcpy(cleanPath, sizeof cleanPath, path);
    convert_path_separators(cleanPath);

    string_path scanPath = { 0 };
    xr_strcpy(scanPath, sizeof scanPath, cleanPath);
    ...
    while ((entry = readdir(dir)) != nullptr)
    {
        ...
        string1024 fullPath;
        xr_strcpy(fullPath, sizeof fullPath, cleanPath);
        if (fullPath[0] && fullPath[xr_strlen(fullPath) - 1] != '/')
            xr_strcat(fullPath, "/");
        ...
        xr_strcpy(findData.name, fullPath);
        ...
        case S_IFREG: findData.attrib = 0;         break;
        default:      findData.attrib = _A_HIDDEN; break;
        }
        restore_path_separators(findData.name);

        bool ignore = false;
        if (m_Flags.test(flNeedCheck))
            ignore = ignore_name(entry->d_name) || ignore_path(fullPath);
        else
            ignore = ignore_name(entry->d_name);
...
void CLocatorAPI::setup_fs_path(pcstr fs_name)
{
    ...
    {
#if defined(XR_PLATFORM_ANDROID)
        pcstr customPathArg = strstr(Core.Params, "-game_path");
        bool customFound = false;
        if (customPathArg)
        {
            string512 cpath;
            if (sscanf(customPathArg, "-game_path \"%511[^\"]\"", cpath) == 1 ||
                sscanf(customPathArg, "-game_path %511s", cpath) == 1)
            {
                string512 ltxPath;
                xr_sprintf(ltxPath, "%s/fsgame.ltx", cpath);
                if (access(ltxPath, F_OK) == 0 || access(cpath, F_OK) == 0)
                {
                    chdir(cpath);
        ...
        if (!customFound && access("/sdcard/OpenXRay/fsgame.ltx", F_OK) == 0)
        {
            chdir("/sdcard/OpenXRay");
            getcwd(full_current_directory, sizeof full_current_directory);
        }
        else if (!customFound && access("/storage/emulated/0/OpenXRay/fsgame.ltx", F_OK) == 0)
        {
            chdir("/storage/emulated/0/OpenXRay");
            getcwd(full_current_directory, sizeof full_current_directory);
        }
        else
#endif
