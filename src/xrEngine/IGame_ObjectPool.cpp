...
void IGame_ObjectPool::prefetch()
{
    ...
    for (const auto& item : sect.Data)
    {
        ...
        p_count++;
        IFactoryObject* fo = NEW_INSTANCE(CLS);
        IGameObject* pObject = fo ? fo->dcast_GameObject() : nullptr;
        if (!pObject && fo)
            pObject = smart_cast<IGameObject*>(fo);
        if (pObject)
        {
...
IGameObject* IGame_ObjectPool::create(pcstr name)
{
    ...
    {
        Msg("! IGame_ObjectPool::create: section '%s' does not exist in system.ltx", name);
        return nullptr;
    }
    if (!pSettings->line_exist(name, "class"))
    {
        Msg("! IGame_ObjectPool::create: section '%s' has no 'class' line", name);
        return nullptr;
    }
    ...
    if (!fo)
    {
        Msg("! IGame_ObjectPool::create: NEW_INSTANCE failed for section '%s' (class '%s')", name, pSettings->r_string(name, "class"));
        return nullptr;
    }
    IGameObject* O = fo->dcast_GameObject();
    if (!O)
        O = smart_cast<IGameObject*>(fo);
    if (!O)
    {
        Msg("! IGame_ObjectPool::create: cast to IGameObject failed for section '%s' (class '%s')", name, pSettings->r_string(name, "class"));
        return nullptr;
    }
