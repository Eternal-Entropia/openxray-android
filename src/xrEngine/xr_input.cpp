...
void CInput::MouseUpdate()
{
    ...
    for (int i = 0; i < count; ++i)
    {
        ...
        {
            if (event.button.button >= 1 && event.button.button <= COUNT_MOUSE_BUTTONS)
            {
                const auto idx = RemapIdx[event.button.button - 1];
                mouseState[idx] = true;
                if (auto* ir = CurrentIR())
                    ir->IR_OnMousePress(IdxToKey[idx]);
            }
            break;
        }
        case SDL_MOUSEBUTTONUP:
        {
            if (event.button.button >= 1 && event.button.button <= COUNT_MOUSE_BUTTONS)
            {
                ...
                if (auto* ir = CurrentIR())
                    ir->IR_OnMouseRelease(IdxToKey[idx]);
            }
            break;
        }
        case SDL_MOUSEWHEEL:
    ...
    if (auto* ir = CurrentIR())
    {
        for (int i = 0; i < MOUSE_COUNT; ++i)
        {
            if (mouseState[i] && mousePrev[i])
                ir->IR_OnMouseHold(IdxToKey[i]);
        }

        if (mouseMoved)
        {
            if (offs[0] || offs[1])
                ir->IR_OnMouseMove(offs[0], offs[1]);

            if (!fis_zero(scroll[0]) || !fis_zero(scroll[1]))
                ir->IR_OnMouseWheel(scroll[0], scroll[1]);
        }
    }
}
    ...
    for (int i = 0; i < count; ++i)
    {
        ...
        case SDL_KEYDOWN:
            if (event.key.repeat)
                continue;
            if (auto* ir = CurrentIR())
                ir->IR_OnKeyboardPress(event.key.keysym.scancode);
            break;

        case SDL_KEYUP:
            if (auto* ir = CurrentIR())
                ir->IR_OnKeyboardRelease(event.key.keysym.scancode);
            break;
        ...
        case SDL_TEXTINPUT:
            ...
            if (auto* ir = CurrentIR())
                ir->IR_OnTextInput(event.text.text);
            break;
...
void CInput::ControllerUpdate()
{
    ...
    for (int i = 0; i < count; ++i)
    {
        ...
        case SDL_CONTROLLERBUTTONDOWN:
            ...
            if (auto* ir = CurrentIR())
                ir->IR_OnControllerPress(ControllerButtonToKey[event.cbutton.button], pressedAxis);
            break;
        ...
        case SDL_CONTROLLERBUTTONUP:
            ...
            if (auto* ir = CurrentIR())
                ir->IR_OnControllerRelease(ControllerButtonToKey[event.cbutton.button], releasedAxis);
            break;
        ...
        case SDL_CONTROLLERSENSORUPDATE:
        {
            ...
            {
                if (auto* ir = CurrentIR())
                    ir->IR_OnControllerAttitudeChange(controllerState.gyroscope);
            }
            break;
        }
        } // switch (event.type)
    ...
    if (auto* ir = CurrentIR())
    {
        for (int i = 0; i < XR_CONTROLLER_BUTTON_COUNT; ++i)
        {
            if (controllerState.buttons[i] && controllerPrev.buttons[i])
                ir->IR_OnControllerHold(ControllerButtonToKey[i], pressedAxis);
        }
    }
    ...
    const auto checkAxis = [this](int axis, const ControllerAxisState& state, const ControllerAxisState& prevState)
    {
        ...
        if (isActive && isPrevActive)
            ir->IR_OnControllerHold(axis, state);
        else if (isActive)
            ir->IR_OnControllerPress(axis, state);
        else if (isPrevActive)
            ir->IR_OnControllerRelease(axis, state);
    };
