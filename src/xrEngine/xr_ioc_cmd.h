...
public:
    ...
    virtual const xr_token* GetToken() const noexcept { return nullptr; }
    virtual bool GetBool() const { return false; }
    virtual float GetFloat(float& min, float& max) const { min = 0.0f; max = 0.0f; return 0.0f; }
    virtual int GetInteger(int& min, int& max) const { min = 0; max = 1; return 0; }
    virtual Fvector* GetFVectorPtr() const { return nullptr; }
...
public:
    ...
    bool GetBool() const override { return value->test(mask); }
    int GetInteger(int& min, int& max) const override { min = 0; max = 1; return value->test(mask) ? 1 : 0; }
...
public:
    ...
    const xr_token* GetToken() const noexcept override { return tokens; }
...
public:
    ...
    float GetFloat(float& fmin, float& fmax) const override
    {
        fmin = min;
        fmax = max;
        return GetValue();
...
public:
    ...
    Fvector* GetFVectorPtr() const override { return value; }
    virtual void Execute(pcstr args)
    {
        Fvector v;
...
public:
    ...
    int GetInteger(int& imin, int& imax) const override
    {
        imin = min;
        imax = max;
        return GetValue();
