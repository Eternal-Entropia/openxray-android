...
class CCC_Start : public IConsole_Command
{
    ...
    void protect_Server_strlwr(pstr str)
    {
        string4096 out;
        xr_strcpy(out, sizeof(out), str);
        xr_strlwr(str);

        pcstr first_slash = strchr(out, '/');
        int len = first_slash ? int(first_slash - out) : int(xr_strlen(out));
        for (int i = 0; i < len; ++i)
        {
            str[i] = out[i];
        }
...
public:
    ...
    virtual void Execute(pcstr args)
    {
        Msg(">>> [CCC_Start] Execute called with args: '%s'", args ? args : "<null>");
        /* if (g_pGameLevel) {
        ...
        protect_Server_strlwr(op_server);
        protect_Name_strlwr(op_client);

        if (!op_client[0] && strstr(op_server, "single"))
            xr_strcpy(op_client, "localhost");

        Msg(">>> [CCC_Start] parsed op_server='%s', op_client='%s', op_demo='%s', g_pGameLevel=%p", op_server, op_client, op_demo, g_pGameLevel);
        ...
        if (g_pGameLevel)
        {
            Msg(">>> [CCC_Start] Deferring KERNEL:disconnect...");
            Engine.Event.Defer("KERNEL:disconnect");
        }

        if (xr_strlen(op_demo))
        {
            Msg(">>> [CCC_Start] Deferring KERNEL:start_mp_demo...");
            Engine.Event.Defer("KERNEL:start_mp_demo", u64(xr_strdup(op_demo)), 0);
        }
        else
        {
            Msg(">>> [CCC_Start] Deferring KERNEL:start...");
            Engine.Event.Defer(
...
public:
    ...
    virtual void Execute(pcstr args)
    {
        Msg(">>> [CCC_Disconnect] Execute called (g_pGameLevel=%p)", g_pGameLevel);
        Engine.Event.Defer("KERNEL:disconnect");
    }
};
//-----------------------------------------------------------------------
