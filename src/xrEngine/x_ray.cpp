...
CApplication::CApplication(pcstr commandLine, GameModule* game, const std::array<RendererModule*, 2>& modules)
{
    ...
    pcstr diffArgs = strstr(Core.Params, "-difficulty ");
    if (diffArgs)
    {
        string64 diff_cmd;
        string64 diff_val;
        if (sscanf(diffArgs + 12, "%63s", diff_val) == 1)
        {
            xr_sprintf(diff_cmd, "g_game_difficulty %s", diff_val);
            Console->Execute(diff_cmd);
        }
    }
