...
bool CResourceManager::_LinkPP(SPass& pass)
{
    ...
    if (GLAD_GL_ARB_separate_shader_objects)
        pp.pp = GLGeneratePipeline(pp.cName.c_str(), pass.ps ? pass.ps->sh : 0, pass.vs ? pass.vs->sh : 0, pass.gs ? pass.gs->sh : 0);
    else
    {
        if (!pass.ps || !pass.vs || !pass.ps->sh || !pass.vs->sh)
            return false;

        const GLuint gs = pass.gs ? pass.gs->sh : 0;
        pp.pp = GLLinkMonolithicProgram(pp.cName.c_str(), pass.ps->sh, pass.vs->sh, gs);
        if (pp.pp)
