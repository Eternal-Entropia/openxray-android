...
BOOL R_constant_table::parse(void* _desc, u32 destination)
{
    ...
    for (GLint i = 0; i < uniformCount; i++)
    {
        ...
        if (r_location == GLuint(-1))
        {
            string256 name_with_bracket;
            xr_sprintf(name_with_bracket, "%s[0]", name);
            r_location = glGetUniformLocation(program, name_with_bracket);
        }
        ...
        case GL_SAMPLER_2D_MULTISAMPLE:
        {
            ...
            if (!C)
            {
                ...
                u16 s_idx = sampler_index++;
                if (destination & RC_dest_all)
                    L.index = s_idx;
                else
                    L.index = u16(s_idx + ((destination & RC_dest_pixel) ? 0 : CTexture::rstVertex));
                L.cls = RC_sampler;
            ...
            break;
        default:
            Msg("! [glr_constants] Unknown or unsupported uniform type 0x%x for '%s'", reg, name);
            bSkip = TRUE;
            break;
        }
        if (bSkip) continue;
