...
#   error No graphics API selected or enabled!
    ...
    void set_Constants(ref_ctable& C) { set_Constants(C._get()); }

    void set_Textures(STextureList* T);
    void set_Textures(ref_texture_list& T) { set_Textures(T._get()); }

    IC void set_Matrices(SMatrixList* M);
    IC void set_Matrices(ref_matrix_list& M) { set_Matrices(M._get()); }

    IC void set_Pass(SPass* P);
    void set_Pass(ref_pass& P) { set_Pass(P._get()); }

    ICF void set_Element(ShaderElement* S, u32 pass = 0);
    void set_Element(ref_selement& S, u32 pass = 0) { set_Element(S._get(), pass); }

    ICF void set_Shader(Shader* S, u32 pass = 0);
    void set_Shader(ref_shader& S, u32 pass = 0) { set_Shader(S._get(), pass); }

    ICF void set_States(SState* _state);
    ICF void set_States(ref_state& _state) { set_States(_state._get()); }
...
#   error No graphics API selected or enabled!
    ...
    ICF void set_c(R_constant* C, Args&&... args)
    {
        ...
        {
            if (C->pp.program && C->pp.program != pp)
                return;
        }
#endif
